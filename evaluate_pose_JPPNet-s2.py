...
def main():
    ...
    pose_out3 = tf.reduce_mean(tf.stack([tf.image.resize(pose_out3_100, tf.shape(image_batch_origin)[1:3,], method=tf.image.ResizeMethod.NEAREST_NEIGHBOR),
                                         tf.image.resize(pose_out3_125, tf.shape(image_batch_origin)[1:3,], method=tf.image.ResizeMethod.NEAREST_NEIGHBOR),
                                         tf.image.resize(pose_out3_075, tf.shape(image_batch_origin)[1:3,], method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)]), axis=0)
    ...
    output_all = tf.expand_dims(output_all, axis=0)
