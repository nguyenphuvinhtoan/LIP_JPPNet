...
def main():
    ...
    image_batch = tf.image.resize(image_batch_origin, [int(h), int(w)])
    image_batch075 = tf.image.resize(image_batch_origin, [int(h * 0.75), int(w * 0.75)])
    image_batch125 = tf.image.resize(image_batch_origin, [int(h * 1.25), int(w * 1.25)])
    ...
    parsing_out1 = tf.reduce_mean(tf.stack([tf.image.resize(parsing_out1_100, tf.shape(image_batch_origin)[1:3,]),
                                           tf.image.resize(parsing_out1_075, tf.shape(image_batch_origin)[1:3,]),
                                           tf.image.resize(parsing_out1_125, tf.shape(image_batch_origin)[1:3,])]), axis=0)
    parsing_out2 = tf.reduce_mean(tf.stack([tf.image.resize(parsing_out2_100, tf.shape(image_batch_origin)[1:3,]),
                                           tf.image.resize(parsing_out2_075, tf.shape(image_batch_origin)[1:3,]),
                                           tf.image.resize(parsing_out2_125, tf.shape(image_batch_origin)[1:3,])]), axis=0)
    parsing_out3 = tf.reduce_mean(tf.stack([tf.image.resize(parsing_out3_100, tf.shape(image_batch_origin)[1:3,]),
                                           tf.image.resize(parsing_out3_075, tf.shape(image_batch_origin)[1:3,]),
                                           tf.image.resize(parsing_out3_125, tf.shape(image_batch_origin)[1:3,])]), axis=0)
