...
def image_scaling(img, label, heatmap):
    ...
    heatmap = tf.image.resize(heatmap, new_shape, method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
...
def random_resize_img_labels(image, label, heatmap, resized_h, resized_w):
    ...
    label = tf.image.resize(tf.expand_dims(label, 0), new_shape, method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
    label = tf.squeeze(label, axis=[0])
    heatmap = tf.image.resize(tf.expand_dims(heatmap, 0), new_shape, method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
...
def resize_img_labels(image, label, heatmap, resized_h, resized_w):
    ...
    label = tf.image.resize(tf.expand_dims(label, 0), new_shape, method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
    label = tf.squeeze(label, axis=[0])
    new_shape = tf.stack([tf.cast(resized_h / 8.0, dtype=tf.int32), tf.cast(resized_w / 8.0, dtype=tf.int32)])
    heatmap = tf.image.resize(tf.expand_dims(heatmap, 0), new_shape, method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
    heatmap = tf.squeeze(heatmap, axis=[0])
    return img, label, heatmap
...
def random_crop_and_pad_image_and_labels(image, label, heatmap, crop_h, crop_w, ignore_label=255):
    ...
    combined_crop = tf.image.random_crop(combined_pad, [crop_h,crop_w,4+NUM_POSE])
    ...
    new_shape = tf.stack([tf.cast(crop_h / 8.0, dtype=tf.int32), tf.cast(crop_w / 8.0, dtype=tf.int32)])
    heatmap = tf.image.resize(tf.expand_dims(heatmap_crop, 0), new_shape, method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
    heatmap = tf.squeeze(heatmap, axis=[0])
...
class LIPReader(object):
    ...
    def __init__(self, data_dir, data_list, data_id_list, input_size, random_scale,
        ...
        self.queue = tf.compat.v1.train.slice_input_producer([self.images, self.labels, self.labels_rev, self.poses], shuffle=shuffle) 
    ...
    def dequeue(self, num_elements):
        ...
        image_batch, label_batch, heatmap_batch = tf.compat.v1.train.batch([self.image, self.label, self.heatmap], num_elements)
